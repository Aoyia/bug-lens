...
import { type EvidenceSummary, type RecordingSession } from "../../shared/protocol";
...
export const RecordPanel = memo(function RecordPanel({
  ...
  return (
    <div className="context-flow" data-testid="record-panel">
      ...
      <div id="url" className="target-url">{activeTab?.url || ""}</div>
      <div id="evidence" className="evidence">
        {activeEvidence(activeSession).map((item) => (
          <span key={item.kind} className={`chip ${item.state}`} title={item.detail}>
