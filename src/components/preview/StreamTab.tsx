...
export const StreamTab = memo(function StreamTab({ snapshot, onSeekVideo, onExportClip }: StreamTabProps) {
  ...
  const nodes = useMemo(() => {
    ...
    if (filterConsole) {
      snapshot.consoleEntries.forEach(item => {
        ...
        const isErrorWarn = level === 'error' || level === 'warn' || level === 'warning';
        if (!filterErrorOnly || isErrorWarn) {
          list.push({ type: 'console', timestamp: item.createdAt, id: `console-${item.id}`, data: item });
        }
      });
    }
    ...
    if (filterNetwork) {
      snapshot.networkEntries.forEach(item => {
        ...
        if (!filterErrorOnly || isError) {
          list.push({ type: 'network', timestamp: item.createdAt, id: `network-${item.id}`, data: item });
        }
      });
    }

    return list.sort((a, b) => a.timestamp - b.timestamp);
  }, [snapshot, filterInteractions, filterConsole, filterNetwork, filterErrorOnly]);

  const originEpochMs = snapshot.session?.timeline.startedAtEpochMs ?? snapshot.session?.timeline.createdAtEpochMs;

  const handleNodeClick = useCallback((id: string, timestamp: number) => {
    setSelectedId(id);
    onSeekVideo?.(timestamp);
  }, [onSeekVideo]);

  const handleClipClick = useCallback((e: Event, timestamp: number) => {
    e.stopPropagation();
      ...
      <div className="stream-header">
        <div className="stream-filter-tags">
          <label className="stream-filter-label">
            <input type="checkbox" checked={filterInteractions} onChange={e => setFilterInteractions(e.currentTarget.checked)} /> 
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3l7 18 3-7 7-3L3 3z"></path></svg>
             交互步骤
          </label>
          <label className="stream-filter-label">
            <input type="checkbox" checked={filterConsole} onChange={e => setFilterConsole(e.currentTarget.checked)} /> 
          ...
          <label className="stream-filter-label">
            <input type="checkbox" checked={filterNetwork} onChange={e => setFilterNetwork(e.currentTarget.checked)} /> 
          ...
          <label className="stream-filter-label error-chip">
            <input type="checkbox" checked={filterErrorOnly} onChange={e => setFilterErrorOnly(e.currentTarget.checked)} /> 
        ...
        {nodes.length === 0 ? (
          ...
          nodes.map(node => {
            ...
            if (node.type === 'interaction') {
              ...
              contentHtml = (
                <>
                  {data.element.text || data.element.tagName} <span className="stream-node-meta">{data.page.url}</span>
                </>
              );
              ...
              contentHtml = (
                <>
                  <strong>{(data.method || 'GET').toUpperCase()}</strong> {data.url}
                </>
              );
            }
            ...
            return (
              ...
              >
                ...
                >
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg>
                  <span>剪辑 5s</span>
                </button>
              </div>
            );
