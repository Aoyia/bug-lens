...
export const NetworkTab = memo(function NetworkTab({
  ...
  const handleRowClick = useCallback((id: string, createdAt: number) => {
    setSelectedId(id);
  ...
  useEffect(() => {
    ...
    const copyButton = detailRef.current.querySelector<HTMLButtonElement>("#btn-copy-curl");
    if (!copyButton) return;

    const handleCopy = () => {
      void copyTextToClipboard(generateCurlCommand(selectedEntry), document).then(() => {
  ...
  return (
    <div className="integrated-card-panel">
      ...
      <div className="network-main-split">
          ...
          {entries.length > 0 ? (
            entries.slice().reverse().map((entry) => {
              ...
              return (
                ...
                >
                  <span className="col-time" title={`绝对时间: ${new Date(entry.createdAt).toLocaleString()}`}>
        ...
        <div id="network-resizer" className="network-resizer" title="拖拽调整列表与详情区域高度"></div>
        <div 
          className="network-detail-panel" 
          ref={detailRef}
          dangerouslySetInnerHTML={{ __html: renderNetworkDetailHtml(selectedEntry) }} 
        />
      </div>
    </div>
  );
