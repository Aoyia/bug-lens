...
1. Scene Alignment: Inspect \`issues/\` screenshots & \`data/session.js\` first to clarify user-indicated abnormal states. The file sets \`window.__BUG_LENS_DATA__ = {...}\` — the JSON object after the \`=\` sign is the data payload. Start by reading the \`summary\` field (top-level digest with counts and failures) and \`screenshotSummaries\` (quick index of all screenshots). Full headers and initiator/call stack details are stored in \`data/network-details.json\` — only read it when deep-diving into a specific request's origin.
...
1. 现场定位：优先查看 \`issues/\` 截图与 \`data/session.js\`，明确用户标记的页面异常状态。该文件设置 \`window.__BUG_LENS_DATA__ = {...}\`，\`=\` 号后的 JSON 对象即为数据载体。请先读取 \`summary\` 字段（顶层摘要，包含计数与失败列表）和 \`screenshotSummaries\`（截图快速索引）。完整请求头、响应头和调用栈详情已独立保存至 \`data/network-details.json\`，仅需深挖某个请求时读取。
...
2. Read \`data/session.js\` — the core data file for AI. It sets \`window.__BUG_LENS_DATA__\` to a JSON object containing session, interaction, console, and network details (AI-friendly, large response bodies are split into \`network/body-*.txt\`). For deep-dive into a specific request (full headers, initiator/call stack), read \`data/network-details.json\` keyed by entry ID.
3. Double-click \`report.html\` in browser for interactive reading (report data is loaded from \`data/report-data.js\`, which contains the full payload).
...
- \`data/session.js\`: AI-friendly core data (JS file setting \`window.__BUG_LENS_DATA__\`). Contains session, interaction, console, and network details in structured JSON. Network headers are trimmed to essentials; full headers + initiator details are in \`data/network-details.json\`.
- \`data/report-data.js\`: Full payload for \`report.html\` (also sets \`window.__BUG_LENS_DATA__\`).
- \`data/network-details.json\`: Full request headers, response headers, and initiator (call stack) for each network request, keyed by entry ID. Only read when deep-diving into a specific request.
...
2. 读取 \`data/session.js\`，这是 AI 核心数据文件。它设置 \`window.__BUG_LENS_DATA__\` 为一个 JSON 对象，包含会话、交互、Console 和 Network 上下文（AI 友好，较长的响应正文已独立保存至 \`network/body-*.txt\`）。如需深挖某个请求的完整头信息和调用栈，请读取 \`data/network-details.json\`（按条目 ID 索引）。
3. 用浏览器双击 \`report.html\`，进行人工交互式查看（报告数据从 \`data/report-data.js\` 加载，包含完整载荷）。
...
- \`data/session.js\`：AI 友好核心数据文件（JS 文件，设置 \`window.__BUG_LENS_DATA__\`），包含会话、交互、Console 和 Network 上下文。Network 头信息已精简为关键字段；完整头信息和调用栈详情见 \`data/network-details.json\`。
- \`data/report-data.js\`：\`report.html\` 使用的完整载荷（同样设置 \`window.__BUG_LENS_DATA__\`）。
- \`data/network-details.json\`：每条 Network 请求的完整请求头、响应头和发起方（调用栈）详情，按条目 ID 索引。仅需深挖某个请求时读取。
...
- \`networkEntries[]\`：录制期间捕获的原始请求 URL、方法、状态、响应头和响应正文。正文状态位于 \`response.bodyStatus\`；较大的正文通过 \`response.bodyPath\` 引用独立文件。Network 头信息已精简为关键字段；完整请求头、响应头和调用栈详情已独立保存至 \`data/network-details.json\`，每条记录仅保留 \`initiatorType\`（如 "script"、"parser"）。
  ...
  const aiNetworkEntries = payload.networkEntries.map((entry) => {
    ...
    const strippedResponse = response
      ? (({
          ...
          if (respRest.headers) {
            respRest.headers = filterHeaders(
              respRest.headers,
              AI_RESPONSE_HEADER_KEEP
            );
          }
          return respRest;
        })(response)
    ...
    return {
      ...
      requestHeaders: filterHeaders(requestHeaders, AI_REQUEST_HEADER_KEEP),
...
export function buildEvidencePackage(
  ...
  const reportScriptTag = `<script src="data/report-data.js"></script>`;
  ...
  const files: EvidencePackageFile[] = [
    ...
    {
      name: "data/report-data.js",
      data: strToU8(
        `window.__BUG_LENS_DATA__ = ${JSON.stringify(reportPayload, null, 2)};`
      ),
    },
    {
      name: "data/network-details.json",
      data: strToU8(JSON.stringify(networkDetails, null, 2)),
    },
    ...networkBodyFiles,
  ];
