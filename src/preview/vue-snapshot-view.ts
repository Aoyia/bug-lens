...
export function renderVueSnapshotMarkup(vue?: VueFrameworkSnapshot): string {
  ...
  const breadcrumbs = chain.map((node) => {
  ...
  let targetDetails = "";
  if (vue.targetComponent) {
    const propsJson = vue.targetComponent.props ? JSON.stringify(vue.targetComponent.props, null, 2) : undefined;
    const stateJson = vue.targetComponent.state ? JSON.stringify(vue.targetComponent.state, null, 2) : undefined;
  ...
  if (vue.stores && vue.stores.length > 0) {
    const storeItems = vue.stores.map((s) => {
