...
export async function putWithinSessionBudget<T extends { sessionId: string }>(
  ...
  return new Promise((resolve, reject) => {
    ...
    sessionRequest.onsuccess = () => {
      ...
      const valueRequest = store.get(
        (value as { id?: IDBValidKey }).id ?? value.sessionId
      );
      valueRequest.onsuccess = () => {
        ...
        if (usedBytes + delta > session.options.maxSessionBytes) {
          sessions.put({
            ...session,
            storage: { usedBytes, limitReached: true },
          });
        ...
        sessions.put({
          ...session,
          storage: { usedBytes: nextUsedBytes, limitReached: isNearLimit },
        });
        result = {
          stored: true,
    ...
    transaction.onabort = () =>
      reject(transaction.error ?? new Error("证据写入事务已中止"));
  });
}
