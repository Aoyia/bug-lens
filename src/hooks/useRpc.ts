...
import { message, type RuntimeMessage, type RuntimeMessageResponseMap } from "../shared/protocol.ts";
...
export function useRpc() {
  ...
  const send = useCallback(async <T extends RuntimeMessage["type"]>(
    type: T,
    ...
    try {
      ...
      if (response && typeof response === "object" && "ok" in response && !(response as { ok: boolean }).ok) {
        ...
        return { ok: false, error: typeof errorVal === "string" ? errorVal : "Unknown error" };
      }
