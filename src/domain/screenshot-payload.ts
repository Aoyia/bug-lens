...
1. 截图与意图分轨识别 (Visual & Intent Identification)：
   ...
   - 需求开发轨 (Feature Development Track)：标注表达“新增按钮/优化布局/添加交互/样式微调”等新需求描述。
     * 非破坏性增量迭代原则：代码修改必须建立在尊重既有逻辑的基础上，优先采用非破坏性（Non-breaking）方式扩展代码，确保改动隔离在目标范围内，严禁破坏或隐式改变任何已有功能的运行方式与逻辑输出（Zero-Regression）。
...
- 零回归与隔离原则：针对已存在的代码模块，任何改动必须严格局部化，保证业务逻辑、交互监听与数据流动完好无损；如果是纯视觉/样式调整，必须严格限制在样式层（Style/Layout）。
