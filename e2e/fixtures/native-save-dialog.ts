...
function run(file: string, args: string[], timeout = 15_000): Promise<{ stdout: string; stderr: string }> {
...
export class MacOSSaveDialogDriver implements NativeSaveDialogDriver {
  ...
  async saveToDirectory(absolutePath: string, timeoutMs = 15_000): Promise<void> {
    if (process.platform !== "darwin") {
      throw new Error("NATIVE_SAVE_UNSUPPORTED: Native save dialog driver is only supported on macOS.");
    }
    ...
    try {
      ...
      throw new Error(`NATIVE_SAVE_FAILED: 无法操作保存对话框。${String(error)}`);
    }
  }
}
...
export function createNativeSaveDialogDriver(browserAppName: string): NativeSaveDialogDriver {
  if (process.platform === "darwin") return new MacOSSaveDialogDriver(browserAppName);
