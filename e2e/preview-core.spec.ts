...
test.describe("Bug Lens Chrome Extension E2E PREV-001: Preview Core Browsing & Timeline Seeking", () => {
  test("PREV-001: complete evidence recording, metrics, video player, 5 view tabs, filtering, seeking and teardown", async ({
    ...
    await targetPage.mouse.move(svgBox!.x + svgBox!.width * 0.2, svgBox!.y + svgBox!.height * 0.2);
    await targetPage.mouse.down();
    await targetPage.mouse.move(svgBox!.x + svgBox!.width * 0.6, svgBox!.y + svgBox!.height * 0.6, { steps: 5 });
    ...
    await expect(targetPage.locator('[data-testid="after-issue-result"]')).toHaveText("普通操作已执行");
    await delay(400);
    ...
    await expect(targetPage.locator('[data-testid="action-status"]')).toContainText("Failure Net 500");
    ...
    logE2e(`${scenarioId}: All target evidence captured cleanly in target page`);

    // (7) 从页面浮层停止前，确保目标页在最前且聚焦，防止失焦产生 VISIBLE_TAB_NOT_ACTIVE
    await targetPage.bringToFront();
    await targetPage.waitForFunction(() => document.hasFocus(), undefined, { timeout: 2_000 }).catch(() => undefined);
    ...
    await expect(previewPage.locator("#meta")).toContainText(persisted.session?.target.initialUrl ?? "http", { timeout: 5_000 });
    await expect(previewPage.locator("#meta")).toHaveText(/\d+\s*秒|时长未知/, { timeout: 5_000 });

    const includedInteractions = persisted.interactions.filter((i) => i.status === "confirmed");
    const capturedScreenshots = includedInteractions.filter((i) => i.screenshot.status === "captured");

    await previewPage.waitForSelector("#metrics .metric", { timeout: 5_000 });

    const metricSteps = await previewPage.locator(".metric-steps strong").innerText();
    const metricDeleted = await previewPage.locator(".metric-deleted strong").innerText();
    const metricScreenshots = await previewPage.locator(".metric-screenshots strong").innerText();
    const metricConsole = await previewPage.locator(".metric-console strong").innerText();
    const metricNetwork = await previewPage.locator(".metric-network strong").innerText();
    const metricIssues = await previewPage.locator(".metric-issues strong").innerText();
    ...
    await delay(300);
    const updatedTime = await video.evaluate((v: HTMLVideoElement) => v.currentTime);
    ...
    const seekedTime = await video.evaluate((v: HTMLVideoElement) => v.currentTime);
    expect(Math.abs(seekedTime - duration * 0.5)).toBeLessThanOrEqual(duration * 0.15 + 0.5);

    const timeTextDisplayed = await previewPage.locator("#video-time-text").innerText();
    expect(timeTextDisplayed).not.toBe("00:00 / 00:00");

    logE2e(`${scenarioId}: Video player controls and seekbar interaction verified`);
    ...
    for (const item of tabs) {
      const tabBtn = previewPage.locator(`.zen-tab-btn[data-tab="${item.key}"]`);
      await tabBtn.click();
    ...
    logE2e(`${scenarioId}: All 5 tabs active and visibility switching verified`);

    // ==========================================
    // 七、问题现场视图与时间跳转
    // ==========================================
    await previewPage.locator('.zen-tab-btn[data-tab="issues"]').click();
    const issueCards = previewPage.locator("#tab-pane-issues .issue-scene-card");
    ...
    const videoTimeAfterSceneSeek = await video.evaluate((v: HTMLVideoElement) => v.currentTime);
    const sessionStartedAt = persisted.session?.timeline.startedAtEpochMs ?? 0;
    const targetSceneTime = (persisted.issueScenes[0]!.observedAtEpochMs - sessionStartedAt) / 1000;
    expect(Math.abs(videoTimeAfterSceneSeek - targetSceneTime)).toBeLessThanOrEqual(3.5);
    ...
    const videoTimeAfterStepSeek = await video.evaluate((v: HTMLVideoElement) => v.currentTime);
    const firstInteractionTime = (includedInteractions[0]!.createdAt - sessionStartedAt) / 1000;
    expect(Math.abs(videoTimeAfterStepSeek - firstInteractionTime)).toBeLessThanOrEqual(3.5);
    ...
    // 1. 选择 error 级别筛选
    const levelSelect = previewPage.locator("#tab-pane-console .panel-filter-select");
    await expect(levelSelect).toBeVisible({ timeout: 5_000 });
    await levelSelect.selectOption("error");

    await previewPage.waitForFunction(() => {
      const rows = Array.from(document.querySelectorAll("#tab-pane-console .console-row"));
      return rows.length > 0 && rows.every((r) => r.classList.contains("console-row-error"));
    }, undefined, { timeout: 3_000 });

    const countTextAfterErrorFilter = await previewPage.locator("#tab-pane-console .panel-filter-count").innerText();
    ...
    await expect(previewPage.locator("#tab-pane-console .console-row")).toHaveCount(1);

    // 3. 搜索无关文本显示明确空态
    await consoleSearchInput.fill("NonExistentConsoleMarker999");
    await expect(previewPage.locator("#tab-pane-console .empty")).toContainText("未找到匹配的 Console 日志");

    // 4. 清空搜索并恢复 all level
    await consoleSearchInput.fill("");
    await levelSelect.selectOption("all");
    await expect(previewPage.locator("#tab-pane-console .console-row")).toHaveCount(persisted.consoleEntries.length);

    // 5. 点击目标 Console Error 行跳转视频时间
    const errorConsoleRow = previewPage.locator('#tab-pane-console .console-row-error').first();
    await errorConsoleRow.click();

    const targetErrorConsoleEntry = persisted.consoleEntries.find((c) => (c.level || "").toLowerCase() === "error");
    expect(targetErrorConsoleEntry).toBeDefined();
    const videoTimeAfterConsoleSeek = await video.evaluate((v: HTMLVideoElement) => v.currentTime);
    const targetConsoleTime = (targetErrorConsoleEntry!.createdAt - sessionStartedAt) / 1000;
    expect(Math.abs(videoTimeAfterConsoleSeek - targetConsoleTime)).toBeLessThanOrEqual(3.5);
    ...
    await previewPage.waitForSelector("#tab-pane-network .network-row", { timeout: 5_000 });

    const initialNetworkRows = previewPage.locator("#tab-pane-network .network-row");
    expect(await initialNetworkRows.count()).toBe(persisted.networkEntries.length);

    // 1. 在 Network 搜索框中输入失败接口路径
    const netSearchInput = previewPage.locator("#tab-pane-network .panel-search-input");
    await netSearchInput.fill("/api/preview/failure");

    await previewPage.waitForFunction(() => {
      const rows = document.querySelectorAll("#tab-pane-network .network-row");
      return rows.length === 1 && rows[0]?.classList.contains("status-error");
    }, undefined, { timeout: 3_000 });

    const countTextAfterNetFilter = await previewPage.locator("#tab-pane-network .panel-filter-count").innerText();
    expect(countTextAfterNetFilter).toContain("匹配");

    // 2. 点击目标 500 网络行展开详情
    const failureNetRow = previewPage.locator('#tab-pane-network .network-row[data-network-id]').first();
    ...
    const targetFailureNetwork = persisted.networkEntries.find((n) => n.url.includes("/api/preview/failure"));
    expect(targetFailureNetwork).toBeDefined();
    const videoTimeAfterNetSeek = await video.evaluate((v: HTMLVideoElement) => v.currentTime);
    const targetNetTime = (targetFailureNetwork!.createdAt - sessionStartedAt) / 1000;
    expect(Math.abs(videoTimeAfterNetSeek - targetNetTime)).toBeLessThanOrEqual(3.5);

    // 4. 清空搜索
    await netSearchInput.fill("");
    await expect(previewPage.locator("#tab-pane-network .network-row")).toHaveCount(persisted.networkEntries.length);
    ...
    await previewPage.waitForSelector("#tab-pane-stream .stream-node", { timeout: 5_000 });

    const totalNodesCount = persisted.interactions.filter((i) => i.status === "confirmed").length +
    ...
    // 打开“仅看错误与异常”
    await previewPage.getByLabel("仅看错误与异常").evaluate((el: Element) => (el as HTMLInputElement).click());

    // 应该只保留 console error/warn 和 network 500
    await previewPage.waitForFunction((total) => {
      const nodes = document.querySelectorAll("#tab-pane-stream .stream-node");
      return nodes.length > 0 && nodes.length < total;
    }, totalNodesCount, { timeout: 3_000 });
    ...
    const firstConsoleErrorNode = previewPage.locator('#tab-pane-stream .stream-node[data-node-id^="console-"]').first();
    await firstConsoleErrorNode.click();
    await expect(firstConsoleErrorNode).toHaveClass(/active/);

    logE2e(`${scenarioId}: Timeline stream tab ordering, error-only filtering and seek verified`);
    ...
    const totalPreviewPages = context.pages().filter((p) => p.url().includes("/preview.html")).length;
