...
import { buildExportManifest, migrateExportPayload, verifyExportIntegrity } from "../src/export/export-manifest.ts";
import { sha256 } from "../src/export/sha256.ts";
import type { RecordingSession } from "../src/shared/protocol.ts";

const session: RecordingSession = {
  id: "session", schemaVersion: 1, extensionVersion: "0.1.0", status: "PREVIEW_READY",
  target: { tabId: 1, initialUrl: "https://example.test", initialTitle: "Example" },
  options: { captureAudio: false, captureVideo: true, captureScreenshots: true, captureConsole: true, captureNetwork: true, captureNetworkBodies: true, privacyMode: "safe", mediaTimesliceMs: 1000, maxSessionBytes: 512 * 1024 * 1024, maxResponseBodyBytes: 2 * 1024 * 1024 },
