...
import { filterNetworkEntries, selectActiveNetworkId } from "../src/preview/network-filter.ts";

import type { NetworkEntry } from "../src/shared/protocol.ts";

const mockNetworkEntries: NetworkEntry[] = [
  { id: "n1", createdAt: 1000, method: "GET", url: "https://api.example.com/v1/users?page=1", status: 200 },
  { id: "n2", createdAt: 2000, method: "POST", url: "https://api.example.com/v1/users", status: 201 },
  { id: "n3", createdAt: 3000, method: "GET", url: "https://cdn.example.com/assets/app.js", status: 304 },
  { id: "n4", createdAt: 4000, method: "DELETE", url: "https://api.example.com/v1/users/42", status: 404 },
  { id: "n5", createdAt: 5000, method: "POST", url: "https://analytics.example.com/collect", status: 500 }
];
...
test("NetworkTab - 5项搜索、大小写忽略、自动重选与空状态测试", () => {
  ...
  assert.deepEqual(searchLower.map((e) => e.id), ["n1", "n2", "n4"]);
  ...
  assert.equal(currentSelectedId, "n5", "排除当前选中项后应自动重选最后一个有效项 (列表翻转展现的首位)");
  ...
  // 4. 无匹配记录
  const noMatch = filterNetworkEntries(mockNetworkEntries, "non-existent-domain.org");
