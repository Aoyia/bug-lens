...
import {
  deriveCacheEvidence,
  isThirdPartyOrFrameworkFrame,
  sliceInitiator,
} from "../src/domain/initiator-slicer.ts";

describe("Initiator 堆栈剪枝与 Cache 来源判定单元测试", () => {
  describe("isThirdPartyOrFrameworkFrame 过滤逻辑", () => {
    it("过滤 chrome-extension 和第三方库帧", () => {
      assert.equal(
        isThirdPartyOrFrameworkFrame("chrome-extension://abc/background.js"),
        true
      );
      assert.equal(
        isThirdPartyOrFrameworkFrame(
          "https://example.com/node_modules/axios/index.js"
        ),
        true
      );
      assert.equal(
        isThirdPartyOrFrameworkFrame(
          "https://example.com/node_modules/core-js/modules/es.promise.js"
        ),
        true
      );
      assert.equal(
        isThirdPartyOrFrameworkFrame("https://example.com/app/main.ts"),
        false
      );
      assert.equal(
        isThirdPartyOrFrameworkFrame(
          "http://localhost:3000/src/services/api.ts"
        ),
        false
      );
    });
  ...
  describe("sliceInitiator 剪枝逻辑", () => {
    it("从堆栈中精准提取首个业务帧，并剔除第三方库帧", () => {
          ...
          callFrames: [
            {
              ...
              url: "https://example.com/node_modules/axios/lib/core/dispatchRequest.js",
              lineNumber: 10,
              columnNumber: 5,
            },
            {
              functionName: "getUserProfile",
              url: "https://example.com/src/services/user-service.ts?token=secret123",
              lineNumber: 41,
              columnNumber: 12,
            },
            {
              functionName: "onClick",
              url: "https://example.com/src/components/UserCard.tsx",
              lineNumber: 87,
              columnNumber: 3,
            },
          ],
      ...
      assert.equal(
        sliced.topFrame.url,
        "https://example.com/src/services/user-service.ts?token=[REDACTED]"
      ); // 验证 URL 中的 Query Value 已被脱敏为 [REDACTED]
    ...
    it("精准提炼 parent 异步起点的业务帧", () => {
          ...
          callFrames: [
            {
              ...
              url: "https://example.com/node_modules/axios/lib/axios.js",
              lineNumber: 5,
              columnNumber: 2,
            },
          ],
            ...
            callFrames: [
              {
                ...
                url: "https://example.com/src/actions/fetch.ts",
                lineNumber: 19,
                columnNumber: 8,
              },
            ],
      ...
      assert.equal(
        sliced.asyncAnchorFrame.url,
        "https://example.com/src/actions/fetch.ts"
      );
    });
    ...
    it("仅含有 url 的简单 initiator 能够提取 topFrame", () => {
      ...
      assert.equal(
        sliced.topFrame?.url,
        "https://example.com/index.html?key=[REDACTED]"
      );
      ...
      const cdpInitiator = {
        ...
        stack: {
          callFrames: [
            {
              functionName: "r.send",
              url: "https://example.com/sdk-glue.js",
              lineNumber: 0,
              columnNumber: 84752,
            },
            {
              functionName: "e",
              url: "https://example.com/bdms.js",
              lineNumber: 0,
              columnNumber: 1199,
            },
          ],
          parent: {
            ...
            callFrames: [
              {
                functionName: "e.event",
                url: "https://example.com/f3ea53a.js",
                lineNumber: 3,
                columnNumber: 909,
              },
            ],
  ...
  describe("deriveCacheEvidence Cache 来源推导", () => {
    ...
    it("正确识别 Service Worker 缓存", () => {
      const cache = deriveCacheEvidence(
        { fromServiceWorker: true, status: 200 },
        false
      );
    ...
    it("正确识别 Prefetch Cache", () => {
      const cache = deriveCacheEvidence(
        { fromPrefetchCache: true, status: 200 },
        false
      );
