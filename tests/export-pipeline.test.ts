...
import { writeEvidenceArchive, type ArchiveSink, type BinaryChunk } from "../src/export/export-pipeline.ts";
...
test("streaming export writes report files and ordered media without a full archive buffer", async () => {
  ...
  const sink: ArchiveSink = {
    ...
    async abort() { assert.fail("archive should not abort"); }
  };
  const records: MediaChunkRecord[] = ["first-", "second", "-third"].map((value, sequence) => ({
    id: `session:${sequence}`,
    sessionId: "session",
    sequence,
    recordedAt: sequence,
  ...
  const archiveSize = output.reduce((total, chunk) => total + chunk.byteLength, 0);
  const archive = new Uint8Array(archiveSize);
  let offset = 0;
  for (const chunk of output) { archive.set(chunk, offset); offset += chunk.byteLength; }
...
test("streaming export aborts its sink when output fails", async () => {
  let aborted = false;
  await assert.rejects(writeEvidenceArchive({
    files: [{ name: "README.md", data: strToU8("evidence") }],
    sessionId: "session",
    mediaSource: { async iterateMediaChunks() { return 0; } },
    sink: {
      async write() { throw new Error("disk full"); },
      async close() { assert.fail("failed output should not close normally"); },
...
test("streaming export writes a manifest after hashing streamed media", async () => {
  ...
  await writeEvidenceArchive({
    ...
    mediaSource: {
      async iterateMediaChunks(_sessionId, visitor) {
        await visitor({ id: "session:0", sessionId: "session", sequence: 0, recordedAt: 0, mimeType: "video/webm", chunk: strToU8("video").buffer as ArrayBuffer });
        return 1;
      }
    },
    sink: { async write(chunk) { output.push(chunk); }, async close() {}, async abort(reason) { throw reason; } },
    createManifest(files) { return { name: "data/manifest.json", data: strToU8(JSON.stringify({ files })) }; }
  });
  const archive = new Uint8Array(output.reduce((total, chunk) => total + chunk.byteLength, 0));
  let offset = 0;
  for (const chunk of output) { archive.set(chunk, offset); offset += chunk.byteLength; }
  const manifest = JSON.parse(strFromU8(unzipSync(archive)["data/manifest.json"]));
