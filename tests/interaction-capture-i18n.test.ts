...
const CAPTURE_SOURCE = resolve(
  process.cwd(),
  "src/recording/interaction-capture.ts"
);
...
// 改动前存在于 interaction-capture.ts 的硬编码中文用户可见错误文案
const HARDCODED_CHINESE = ["交互写入未完成："];

const I18N_KEYS = ["cleanupInteractionWriteFailed"];

test("交互写入失败明细必须走 i18n（禁止硬编码中文）", () => {
  const source = readFileSync(CAPTURE_SOURCE, "utf8");
  for (const literal of HARDCODED_CHINESE) {
    assert.ok(
      !source.includes(literal),
      `interaction-capture.ts 不得硬编码 '${literal}'，应通过 t() 提供`
    );
  }
});

test("交互写入失败新增 i18n 词条必须双语齐全且 en 无中文字符", () => {
