...
import { defaultAnnotation, markIssueSceneResult, normalizeAnnotation, withIssueNarrative } from "../src/domain/issue-scene.ts";
...
test("domain/issue-scene functions create and normalize scene properties", () => {
  const annotation = defaultAnnotation({ clientX: 192, clientY: 108 }, { width: 1920, height: 1080 });
  ...
  const withNarr = withIssueNarrative(baseScene, { actual: "Button broke", expected: "Button works" }, normalized);
...
test("domain/issue-scene normalizes userAnnotations and builds target snapshot", async () => {
  const { buildTargetSnapshot, normalizeAnnotation } = await import("../src/domain/issue-scene.ts");
  ...
  const normalized = normalizeAnnotation({
    ...
    userAnnotations: [
      { type: "rect", color: "", xRatio: -0.1, yRatio: 0.2, widthRatio: 1.5, heightRatio: 0.4 },
      { type: "arrow", color: "#000", start: { xRatio: 0, yRatio: 0 }, end: { xRatio: 1, yRatio: 1 } }
    ]
  });
