...
import { formatMacOSInputPermissionError, parseChromeShortcut } from "../e2e/fixtures/native-shortcut.ts";
...
test("parseChromeShortcut rejects missing and unsupported keys", () => {
  ...
  assert.throws(() => parseChromeShortcut("Ctrl+F12"), /ACTION_SHORTCUT_UNSUPPORTED/);
  assert.throws(() => parseChromeShortcut("Ctrl+Y+Z"), /ACTION_SHORTCUT_UNSUPPORTED/);
});
