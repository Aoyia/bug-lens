...
test("db storage policy and commands management", async () => {
  await db.saveStoragePolicy({ retentionDays: 14, maxSessionBytes: 200 * 1024 * 1024, maxResponseBodyBytes: 2 * 1024 * 1024, compression: "balanced" });
  ...
  const claimedCmd = await db.claimCommand({ commandId: "cmd-1", kind: "start", sessionId: "sess-1", createdAtEpochMs: Date.now() });
...
test("db session claim, active clear and update session flow", async () => {
  const session: RecordingSession = {
    ...
    status: "RECORDING",
    target: { tabId: 10, initialUrl: "https://example.test", initialTitle: "DB Test" },
    options: {
      captureAudio: false, captureVideo: true, captureScreenshots: true, captureConsole: true, captureNetwork: true, captureNetworkBodies: true, privacyMode: "safe", mediaTimesliceMs: 1000, maxResponseBodyBytes: 1024, maxSessionBytes: 100 * 1024 * 1024
    },
  ...
  await db.updateSession("sess-db-1", (curr) => ({ ...curr, status: "PREVIEW_READY" }));
...
test("db interaction, console, network and media chunks CRUD and budget tests", async () => {
  ...
  const session: RecordingSession = {
    ...
    extensionVersion: "0.1.0",
    status: "RECORDING",
    target: { tabId: 11, initialUrl: "https://example.test", initialTitle: "CRUD Test" },
    options: {
      captureAudio: false, captureVideo: true, captureScreenshots: true, captureConsole: true, captureNetwork: true, captureNetworkBodies: true, privacyMode: "safe", mediaTimesliceMs: 1000, maxResponseBodyBytes: 1024, maxSessionBytes: 100 * 1024 * 1024
    },
    timeline: { createdAtEpochMs: Date.now() },
  ...
  await db.saveInteraction({
    ...
    coordinates: { clientX: 0, clientY: 0, pageX: 0, pageY: 0, scrollX: 0, scrollY: 0, devicePixelRatio: 1, viewport: { width: 100, height: 100 } },
    element: { tagName: "button", classNames: [], attributes: {}, locators: [] },
  ...
  await db.updateNetworkEntryWithinBudget(`${sessionId}:req-1`, (curr) => ({ ...curr, status: 200 }));
