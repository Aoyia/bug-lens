...
import type { InteractionRecord, RecordingSession } from "../src/shared/protocol.ts";
...
const interaction: InteractionRecord = {
  ...
  element: { tagName: "BUTTON", classNames: [], attributes: {}, boundingBox: { x: 0, y: 0, width: 10, height: 10 }, locators: [] },
...
test("interaction capture serializes accepted events behind one capture interface", async () => {
  ...
  await capture.handle(interaction, { tab: { id: 7 } } as chrome.runtime.MessageSender);

  assert.equal(stored?.sessionId, session.id);
  assert.equal(stored?.screenshot.status, "disabled");
  assert.deepEqual(sessionEvents, [{ type: "quality-delta", delta: { interactionCount: 1, confirmedInteractionCount: 0 } }]);
