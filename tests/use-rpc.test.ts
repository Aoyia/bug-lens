...
async function _typeCheckAssertions() {
  ...
  if (statusRes.ok) {
    type _T1 = Expect<
      Equal<typeof statusRes.data, { ok: true; session?: RecordingSession }>
    >;
  }

  // 2. session/start
  const startRes = await send("session/start", {
    tabId: 1,
    options: {} as any,
    commandId: "cmd",
  });
  if (startRes.ok) {
    type _T2 = Expect<
      Equal<typeof startRes.data, { ok: true; session: RecordingSession }>
    >;
  }
  ...
  if (listRes.ok) {
    type _T3 = Expect<
      Equal<typeof listRes.data, { ok: true; sessions: SessionOverview[] }>
    >;
  }
  ...
  if (storageRes.ok) {
    type _T4 = Expect<
      Equal<typeof storageRes.data, { ok: true; storage: StorageOverview }>
    >;
  }
  ...
  if (deleteRes.ok) {
    type _T5 = Expect<
      Equal<typeof deleteRes.data, { ok: true; deleted: boolean }>
    >;
  }
}
...
test("useRpc - background 返回 { ok: false, error: '...' }", async () => {
  ...
  const res = await rpcInstance.send("session/start", {
    tabId: 1,
    options: {} as any,
    commandId: "c1",
  });
